...
def image_upload_url(recipe_id):
    """Create an dreturn a recipe detail URL."""
...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_full_update(self):
        ...
        recipe.refresh_from_db()
        for k, v in payload.items():
            self.assertTrue(getattr(recipe, k), v)
    ...
    def _test_recipe_other_users_recipe_error(self):
    ...
    def test_create_recipe_with_exixting_tag(self):
        """Test to create a recipee with existing tag"""
        tag_kenya = Tag.objects.create(user=self.user, name="Pilau")
        payload = {
            "title": "Sample recipe update",
            "time_minutes": 21,
            "price": Decimal("5.25"),
            "tags": [{"name": "Pilau"}, {"name": "Dinner"}],
        }
        res = self.client.post(RECIPE_URL, payload, format="json")
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        recipes = Recipe.objects.filter(user=self.user)
        ...
        recipe = recipes[0]
        self.assertEqual(recipe.tags.count(), 2)
        self.assertIn(tag_kenya, recipe.tags.all())
        for tag in payload["tags"]:
            exists = recipe.tags.filter(
                name=tag["name"],
                user=self.user,
    ...
    def test_create_recipe_with_new_igredient(self):
        """Test create a recipe with an new ingredient"""
        payload = {
            "title": "Sample recipe update",
            "time_minutes": 21,
            "price": Decimal("5.25"),
            "ingredients": [{"name": "Floor"}, {"name": "Sugar"}],
        }
    ...
    def text_create_recipe_with_existing_ingredient(self):
    ...
    def test_filter_by_ingredients(self):
        r1 = create_recipe(user=self.user, title="Thai Vegetable Curry")
        r2 = create_recipe(user=self.user, title="Eggplant with Tahini")
        ing1 = Ingredient.objects.create(user=self.user, name="Vegan")
        ing2 = Ingredient.objects.create(user=self.user, name="Vegetarian")
